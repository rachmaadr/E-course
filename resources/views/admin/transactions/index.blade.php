        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                @forelse ($transactions as $transaction)
                    <div class="item-card flex flex-row items-center p-4 border rounded-lg shadow space-x-4">
                        ...
                        <div class="flex-1 text-center">
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $transaction->created_at->format('d-m-y') }}</h3>
                        </div>
                        <div class="flex-1 text-center">
                            <p class="text-slate-500 text-sm mb-3">Status</p>
                            @if ($transaction->is_paid)
                                <span class="text-sm font-bold py-2 px-3 rounded-full bg-green-500 text-white">Active</span>
                            @else
                                <span class="text-sm font-bold py-2 px-3 rounded-full bg-orange-500 text-white">Pending</span>
                            @endif
                        </div>
                        <div class="hidden md:flex flex-col text-center flex-shrink-0 mx-4">
                            <p class="text-slate-500 text-sm">Student</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $transaction->user->name }}</h3>
                        </div>
                        <div class="hidden md:flex flex-row items-center gap-x-3 text-center ml-auto">
                            <a href="{{ route('admin.subcription_transactions.show', $transaction) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                View Details
                            </a>
                        </div>
                    </div>
                @empty
                    <p class="text-center">Transaction is not available</p>
                @endforelse
            </div>
        </div>
    </div>
